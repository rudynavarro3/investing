...
for idx, year in enumerate(years):
    ...
    past_days = 720
    locations = ["Oklahoma City, OK", "Seward, OK", "Guthrie, OK", "Edmond, OK", "Norman, OK", "Moore, OK"]
    for location in locations:
        if update_historical:
            if 'sold' not in locals():
                sold = scrape_property(radius=radius, location=location, listing_type="sold", date_from=date_from, date_to=date_to)
                print(f"Number of sold properties in {location}: {len(sold)}")
            else:
                df = scrape_property(radius=radius, location=location, listing_type="sold",date_from=date_from, date_to=date_to)
                print(f"Number of sold properties in {location}: {len(df)}")
                sold = pd.concat([sold,df])
        if year == years[-1]:
            if 'selling' not in locals():
                selling = scrape_property(radius=radius, location=location, listing_type="for_sale", date_from=date_from, date_to=date_to)
                print(f"Number of selling properties in {location}: {len(selling)}")
            else:
                df = scrape_property(radius=radius, location=location, listing_type="for_sale", date_from=date_from, date_to=date_to)
                print(f"Number of selling properties in {location}: {len(df)}")
                selling = pd.concat([selling,df])

            if 'renting' not in locals():
                renting = scrape_property(radius=radius, location=location, listing_type="for_rent", date_from=date_from, date_to=date_to)
                print(f"Number of renting properties in {location}: {len(renting)}")
            else:
                df = scrape_property(radius=radius, location=location, listing_type="for_rent", date_from=date_from, date_to=date_to)
                print(f"Number of renting properties in {location}: {len(df)}")
                renting = pd.concat([renting,df])

            if 'pending' not in locals():
                pending = scrape_property(radius=radius, location=location, listing_type="pending", date_from=date_from, date_to=date_to)
                print(f"Number of pending properties in {location}: {len(pending)}")
            else:
                df = scrape_property(radius=radius, location=location, listing_type="pending", date_from=date_from, date_to=date_to)
                print(f"Number of pending properties in {location}: {len(df)}")
                pending = pd.concat([pending,df])

    if update_historical:
        sold.to_csv(f"data/homeHarvest/HomeHarvest_OKC_{year}_sold.csv", index=False)
        del sold
